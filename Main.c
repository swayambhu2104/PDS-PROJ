...
void loadGame(GameState* game)
{
	...
	//Load images and create rendering textures from the
	SDL_Surface* Surface = NULL;
	Surface = IMG_Load("images/gwen.png");
	if (Surface == NULL)
	{
		printf("Cannot find obstacle!\n\n");
		SDL_Quit();
		exit(1);
	}
	...
	Surface=IMG_Load("images/2.png");
	if (Surface == NULL)
	{
		printf("Cannot find man!\n\n");
		SDL_Quit();
		exit(1);
	}
	...
	Surface = IMG_Load("images/1.png");
	if (Surface == NULL)
	{
		printf("Cannot find man's pose!\n\n");
		SDL_Quit();
		exit(1);
	}
	...
	//Load fonts
	game->font = TTF_OpenFont("font.ttf", 48);
	if (!game->font)
	{
	...
	//Load sounds
	game->bgMusic = Mix_LoadWAV("sounds/bg_new.wav");
	if (game->bgMusic != NULL)
	{
		Mix_VolumeChunk(game->bgMusic, 500);		//64
	}

	game->dieSound = Mix_LoadWAV("sounds/kill3.wav");
	game->jumpSound = Mix_LoadWAV("sounds/jump2.wav");
	game->landSound = Mix_LoadWAV("sounds/jump4.wav");


	game->label = NULL;
	game->label2 = NULL;

	game->man.x = 100;				//320-40
	game->man.y = 240 - 40;
	game->man.dy = 0;
	game->man.dx = 0;
	game->man.onLedge = 0;
...
//useful utility to see if two rectangles are colliding or not
...
void collisionDetect(GameState* game)
{
	for (int i = 0; i < NUM_STARS; i++)
	{
		if (collide2d(game->man.x, game->man.y, game->stars[i].x, game->stars[i].y, 48, 48, 32, 32))		//48,48,32,32
		{
			if (!game->man.isDead)
			{
				game->man.isDead = 1;
	...
	//check for collisions with any ledges (brick blocks)
...
void doRender(SDL_Renderer* renderer, GameState* game)
{
	...
	else if (game->statusState == STATUS_STATE_GAME)
	{
		...
		SDL_Rect rect = { game->scrollX + game->man.x, game->man.y, 100, 48 };
		SDL_RenderCopyEx(renderer, game->manFrames[game->man.animFrame],
			NULL, &rect, 0, NULL, (game->man.facingLeft == 1));

		if (game->man.isDead)
		{
			SDL_Rect rect = { game->scrollX + game->man.x  + 38 / 2 + 10, game->man.y  - 83 / 2, 38, 83 };
